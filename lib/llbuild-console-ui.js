...
class LLBuildConsoleUI {
    ...
    getTaskListFromBuilder(target, targetName) {
        if (target === null || target === undefined || target === true || target === false) {
            return [];
        } else if (target.constructor === Array) {
            return LLBuildConsoleUI.flattenTaskTree(target.map((t, idx) => this.getTaskListFromBuilder(t, `${targetName}[${idx}]`)));
        } else if (target.constructor === String) {
            return LLBuildConsoleUI.flattenTaskTree([this.getTaskListFromBuilder(this.builder.targets[target], target)]);
    ...
    static flattenTaskTree(src) {
        ...
        if (src.constructor !== Array) {
            if (src.constructor !== String) {
                console.error(`unexpected object in task tree: ${JSON.stringify(src)} of type ${typeof(src)}`);
            }
            return [src];
        }

        const flatChildren = src.map(c => LLBuildConsoleUI.flattenTaskTree(c));
        let l = [];
